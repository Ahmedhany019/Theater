  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5'>
      ...
      ${isOpen ? 'max-md:w-full' : 'max-md:w-0'}`}>
        <XIcon onClick={() => {setIsOpen(!isOpen)}} className='md:hidden w-6 h-6 cursor-pointer absolute top-6 right-6'/>
        <Link onClick={() => {scrollTo(0, 0),setIsOpen(false)}} to="/">Home</Link>
        <Link onClick={() => {scrollTo(0, 0),setIsOpen(false)}} to="/movies">Movies</Link>
        <Link onClick={() => {scrollTo(0, 0),setIsOpen(false)}} to="/">Theaters</Link>
        <Link onClick={() => {scrollTo(0, 0),setIsOpen(false)}} to="/my_bookings">My Bookings</Link>
      </div>
        ...
        {!user ? (
          ...
          <UserButton>
            <UserButton.MenuItems>
              <UserButton.Action label='My Bookings' labelIcon={<TicketPlus width={15} />} onClick={() => navigate('/my_bookings')}/>
            </UserButton.MenuItems>
          </UserButton>
        )}
